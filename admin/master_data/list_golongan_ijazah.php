  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="card col">
            <div class="card-header"><h5 class="float-left">Data Golongan Ijazah</h5> <button class="float-right btn btn-success" data-toggle="modal" data-target="#modal-add-pangkat"><i class="fa fa-plus"></i> Tambah data</button></div>
            <div class="card-body">
              <!-- get list data pangkat / golongan -->
                <?php 
                use Lib\Database\Master;
                $golIjazahObj = new Master;
                $dataGolIjazah = $golIjazahObj->getGolonganIjazah();
                ?>
              <!-- get list data pangkat / golongan -->
              <table class="table table-striped table-bordered" id="table">
                ...
                <tbody>
                  <?php $n=0; while($row = $dataGolIjazah->fetch_assoc()){$n++; ?>
                  <tr>
                    <td><?=$n?></td>
                    <td><?=$row['golongan']?></td>
                    ...
                    <td>
                      <div class="btn-group">
                        <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#editModal" onClick="fill_edit_form('<?=$row['id']?>','<?=$row['golongan']?>','<?=$row['keterangan']?>')"><i class="fa fa-edit"></i></button>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="<?=BASE_URL?>/admin/master_data/act_edit_golongan_ijazah.php" method="POST">
      <div class="modal-body">
        <div class="form-group">
          <label for="">Golongan</label>
          <input type="text" value="" id="editFormPangkat" name="golongan" class="form-control">
        </div>
        <div class="form-group">
          <label for="">Keterangan</label>
          <input type="text" id="editFormKeterangan" class="form-control" name="keterangan">
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="<?=BASE_URL?>/admin/master_data/act_add_golongan_ijazah.php" method="POST">
      <div class="modal-body">
        <div class="form-group">
          <label for="">
            Golongan
          </label>
          <input type="text" value=""  name="golongan" class="form-control">
        </div>
...
<script>
  $('#table').DataTable();
  function fill_edit_form(id,nama, keterangan){
    $('#editFormPangkat').val(nama)
    $('#editFormKeterangan').val(keterangan)
    $('#editFormId').val(id)
  }
...
function deleteData(id){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        window.location.href = '<?=BASE_URL?>/admin/master_data/delete_golongan_ijazah.php?id='+id
